...
public class CompositionJFrame extends LinksJFrame {
	...
	public CompositionJFrame (Project project, ClassDrawing drawClass) {
		super("Composition", drawClass, "./Images/compo.jpg", false);
		...
		for(int i=0 ; i < project.getProjectProgram().getAllPackages().size() ; i++){
			...
			for (int j = 0 ; j < actualPackage.getAllClass().size() ; j++){
				...
				for (int k = 0 ; k < links.size() ; k++){
					if (links.get(k).getLink() == LinkEnum.COMPOSITION){
	...
	private class AddListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			Link aggregation = new Link(LinkEnum.COMPOSITION, classToToAdd, classFromToAdd);
